...
export default function TimingsPage() {
  ...
  const fetchCourts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/turfs`); // Adjust endpoint as needed
  ...
  const fetchCourtAvailability = async (courtId) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/turfs/court-timings/${courtId}`
      );
  ...
  const saveTimings = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/turfs/update-timings/${selectedCourtId}`,
        {
